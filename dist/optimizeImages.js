...
if (process.argv.length === 3) {
    ...
    console.error("next-image-export-optimizer: Breaking change: Please provide the path to the next.config.[js/ts] file as an argument with the name --nextConfigPath.");
    // Reset the color
...
if (nextConfigPath) {
    ...
    if (fs.existsSync(jsConfigPath)) {
        ...
        console.error("\x1b[31m");
        console.error("next-image-export-optimizer: Could not find next.config.js, next.config.ts, or next.config.mjs. Please provide the path to the configuration file.");
        console.error("\x1b[0m");
        process.exit(1);
    }
}
...
const nextImageExportOptimizer = async function () {
    ...
    try {
        ...
        if (nextjsConfig.transpilePackages === undefined || // transpilePackages is not set
            ...
            console.warn("\x1b[41m", `Changed in 1.2.0: You have not set transpilePackages: ["next-image-export-optimizer"] in your next.config.[js/ts]. This may cause problems with next-image-export-optimizer. Please add this line to your next.config.[js/ts].`, "\x1b[0m");
        }
    }
    catch (e) {
        ...
        const result = await (0, getRemoteImageURLs_1.getRemoteImageURLs)(remoteImageFileName, nextConfigFolder, folderPathForRemoteImages);
    ...
    const allFilesInRemoteImageFolder = fs.existsSync(folderNameForRemoteImages)
        ? fs.readdirSync(folderNameForRemoteImages)
        : [];
    const encodedRemoteImageURLs = remoteImageURLs.map((url) => urlToFilename(url));
    function removeLastUpdated(str) {
    ...
    for (const filename of allFilesInRemoteImageFolder) {
        if (encodedRemoteImageURLs.includes(filename) ||
        ...
        console.log(`Deleted ${filename} from remote image folder as it is not retrieved from ${remoteImageFileName}.`);
    }
    ...
    if (!isImageFolderSubdirectoryOfPublicFolder) {
        console.warn("\x1b[41mWarning: The image folder is not a subdirectory of the public folder. The images in the image folder are not optimized.\x1b[0m");
    }
